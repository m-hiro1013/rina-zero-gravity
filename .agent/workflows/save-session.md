...
## Step 2: 既存の prompt フォルダファイルの確認 📂 (※超重要：削除・上書き厳禁)

情報を書き込む前に、**必ず**既存の `prompt` フォルダ内の各管理ファイル（`WORKFLOW.yaml`, `KNOWLEDGE.md`, `ARCHITECTURE.yaml`, `DATABASE.md` 等）の現在の内容をツール等で読み込み、内容を完全に把握すること。

### 🚨 累積整合性チェック (Cumulative Integrity Check)
書き込みプランを立てる際、以下の項目が **「既存の内容をすべて含んでいるか」** を1行ずつ突き合わせ確認せよ。
1. **decisions.adopted**: 過去の全決定（D001〜）が維持されているか？
2. **features.completed**: 過去の全実績（F001〜）が維持されているか？
3. **cautions**: 過去の全注意点が維持されているか？
4. **progress.completed_phases**: 過去の全フェーズ履歴が維持されているか？

**「今回の分だけで上書きする」ことは重大なルール違反であり、二度と繰り返してはならない。** 既存情報の末尾に新情報を「追記」する形になっているか、指差し確認を徹底すること。

## Step 3: WORKFLOW.yaml を更新 (※累積・蓄積型を絶対死守)
