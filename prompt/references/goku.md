...
- **Session 24 (2026-02-19)**: TOON配信パイプラインの構築と `new-world` への統合完遂。
  - **技**: データを分離して Firebase 経由で PUSH する並列配信アーキテクチャ。
  - **学び**: GASの実行制限を2段階トリガーで突破する DevOps テクニック。
- **Session 25 (2026-02-28)**: `sumiyaki-sake` イベントLPの内容ブラッシュアップ。
  - **技**: 蔵元公式サイトからのストーリー抽出と、モバイル対応の視覚表現最適化。
  - **学び**: 提示されたスペック数値を独断で変更しない「スペック遵守の絶対鉄則」。
- **Session 27 (2026-02-28)**: `save-session` における致命的な上書きミスの克服。
  - **技**: **記憶累加の原則（Principle of Accumulation）**。書き込み直前の「整合性・生存確認」の強制。
  - **学び**: 記憶の上書きはプロジェクトの破産であり、エージェントの死。「追記のみ」を絶対の掟とする。 [+20]
...
- セクションログは追記のみ、削除・置換禁止。過去の `decisions`, `features`, `history` を消すことは「プロジェクトの破産」である [+20] (2026-02-28)
- 書き込み直前に既存データがすべて残っているか、一行ずつ「物理的な生存確認」を義務付ける [+10] (2026-02-28)
...
- ユーザーへのヒアリングは「一問一答（3案＋おすすめ提示）」形式で行うことで、決定精度と速度を最大化する [+10] (2026-02-28)
- 複雑な成果物を量産する前に、必ず「先行1枚サンプル」で品質を確定させるフェーズを設ける [+5] (2026-02-28)
...
- 汎用知見ベースへのパスを絶対パスでハードコードし、プロジェクト間の知見断絶を防ぐ [+3] (2026-02-06)
- セッション状態と知識ベースを分離し、コンテキスト溢れを防ぐ [+3] (2026-01-27)

### React / 次世代スタイリング 🆕

- Tailwind v4 + Vite v7 環境では `@tailwindcss/vite` を使用し、PostCSS 競合問題を解決する [+3] (2026-02-10)
- Hooks（useMemo/useState等）は必ず早期 return より上に記述。非決定的なフック呼び出し順序は絶対禁止 [+3] (2026-02-10)
- ダッシュボードのヘッダーには「フィルタ条件/選択対象」を常に明示し、ユーザーの迷子を防ぐ [+2] (2026-02-10)
- 複雑な情報構造では、カテゴリ別の横並びタブを採用し、データへのアクセス密度と一覧性を両立させる [+2] (2026-02-10)
...
- POSなど分散登録環境では「コード単体」での集計はコード再利用により崩壊する。必ず「店舗CD」等と複合キー化する [+3] (2026-02-27)
- データの二重管理を防ぐため、GASから別プロジェクトのマスタを `SpreadsheetApp.openById()` で直接ルックアップする設計は強力 [+3] (2026-02-27)

### UI設計とデータ統合 🆕

- マスター入力画面で「業態」などの分類はテキスト入力ではなく`select`タグを使い、意図しない分散（表記ゆれ）を未然に防ぐ [+3] (2026-02-27)
...
- StartupフェーズでCommit分類とReadiness Checkを徹底し、初動のミスを防ぐ [+3] (2026-02-06)
...
- **スペック遵守の絶対鉄則**: ユーザーから提示された数値や仕様（「30種類」等）は、AI側の主観的判断で勝手に「より良く見える」ように書き換えてはならない。物理的・契約的制約が背景にある可能性を常に考慮し、そのまま出力することを最優先する [+10] (2026-02-28)
...
- macOS等の権限エラーを防ぐため、コマンド提示時は必ず先頭に `cd` を含める [+5] (2026-02-28)

### AI 透明性・モデル最適化 🆕

- ユーザーの信頼性確保のため、AIの思考プロセスや実行した検索クエリをUIで可視化する [+4] (2026-02-08)
- Gemini 3 FlashのFunction Callingでは `thoughtSignature` を履歴に保持・返送することが400エラー回避の絶対条件 [+5] (2026-02-08)
- 複雑なリサーチには `maxSteps` を適切に設定（10ステップ程度）し、多段ツール呼び出しを許容する [+3] (2026-02-08)
- マルチモーダルデータ（画像・ファイル）は `parts` 配列として一貫して管理・送信する [+3] (2026-02-08)
...
- 画像生成において特定の背景素材（実物資産）を維持する場合はAIリメークを禁止し、画像編集レベルの加工に限定するプロンプトを構築する [+8] (2026-02-28)
- 日本語の文字入れ精度を上げるには、プロンプト内で綴りを一文字ずつ指定し、正確性を強調（Precise rendering）する [+5] (2026-02-28)
...
### b-log RPA Enhancement & Element Identification Strategy 🆕
- [RPA-Robustness] Pivot方式（ターゲット要素を軸に tr:has() で親の行を特定）は、IDが不安定な一覧画面での要素操作における最強の安定策。 [+5] (2026-02-11)
- [RPA-Latency] UI操作（ラジオボタン等）後の disabled 解除には微小なタイムラグがある。Playwrightの自動待機を過信せず is_enabled() ループによる「粘り強い待機」が成功率を左右する。 [+4] (2026-02-11)
- [RPA-Governance] うまくいかない時の勝手な推測によるコード変更は絶対禁止。既存スキルの再確認と、b-log（事実）の解析を最優先する。事実に勝る推測なし。 [+5] (2026-02-11)
- [RPA-Skill] ログイン直後の店舗選択のように、共通画面での停止を防ぐ Skill は、プロジェクト全体の「初動の迷い」を解決する。 [+3] (2026-02-11)
- [RPA-Iframe] isInIframe, iframeId, iframeName を記録することで、Selenium/Playwright で画面切り替えを自動挿入できる。 [+3] (2026-02-11)
- [RPA-Fallback] ID → ユニークCSS → XPath → テキスト → 座標の5段階フォールバックにより、どんな要素でも確実にアクセスできる。 [+5] (2026-02-11)
- [RPA-IframeReturn] iframe内操作後の保存完了モーダル等はメインフレーム側で処理し、親画面への帰還を忘れない。 [+4] (2026-02-11)

### LP（ランディングページ）作成の基本フロー 🆕

- ビルド不要の「HTML/CSS/JS（Vanilla）」構成をデフォルトのLPテンプレートとする [+3] (2026-02-24)
- サーバーホスティング時にそのままポン置き（index.html + images/）できる構成を維持する [+3] (2026-02-24)
- 開発テスト用に簡易サーバー（serve等）だけはpackage.jsonに定義しておくことで、fileプロトコルのCORSエラーなどを防ぐ [+2] (2026-02-24)
- 「LP作成をして」と言われたら、まずはこの構成案で進めることを基本方針とする [+3] (2026-02-24)
- **ストーリーテリングによる信頼性**: 蔵元の歴史や、仕込み水の硬度といった「具体的かつ情緒的な背景」をLPに盛り込むことで、ただの宣伝ではない「読み物」としての深みと信頼性が生まれる [+3] (2026-02-28)
- **熱燗訴求のベネフィット化**: 「お燗場」という手段を「熱燗も楽しめる！」というベネフィット（体験）として表現することで、ユーザーの期待値を直感的に高められる [+2] (2026-02-28)
