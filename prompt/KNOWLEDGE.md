- セッション: 30 (sumiyaki-sake LP Refinement & Spec Accuracy Enforcement)
  ...
  - insights:
    ...
    - [Workflow-Design] **コンテキストの物理的・論理的分離**: 進捗（WORKFLOW.yaml）はプロジェクト個別に閉じ、反省・教訓（KNOWLEDGE.md）はグローバルに集約する。これにより、プロジェクトの機密性を守りつつ、AIの「知恵」だけを全プロジェクトで共有・進化させることができる。 [+10]
